defmodule SmmMonitor.TUI.Renderers.RatatouilleRenderer do
  @moduledoc """
  Draws the dashboard with Ratatouille.
  ...
  Everything this module does is presentation; the numbers, slicing and
  scroll bounds all come from `SmmMonitor.TUI.Model`.
  """
  ...
  defp summary(model) do
    ...
    panel(title: "last #{window_label(model.window_ms)} · #{model.tab}", height: 5, padding: 0) do
      label do
      ...
      label do
        text(content: "  positive #{percentages.positive}% (#{stats.positive})", color: @positive)
        text(content: "   neutral #{percentages.neutral}% (#{stats.neutral})", color: @neutral)
        text(content: "   negative #{percentages.negative}% (#{stats.negative})", color: @negative)
      end
    end
  end
